...
import { BookOpen, ChevronRight, GraduationCap, Clock, CheckCircle2, User, LogOut, Mail, KeyRound, Loader2, Lock, X, ShieldCheck, Timer, AlertCircle, Bookmark, ArrowLeft, History, Trophy, Target, Zap, Shield, Activity, Edit2, Check, Scissors, FileText, Brain } from "lucide-react";
...
function SimuladoContent() {
           ...
           <div className="flex-1 overflow-y-auto p-6 pb-48 sm:p-16 bg-white/[0.01] custom-scrollbar">
              <div className="max-w-3xl mx-auto relative">
                ...
                <div className="space-y-1">
                 ...
                 {estudoMode && showFeedback[currentQuestion] && (q.explicacao || q.explicação || q.explicacão) && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                      <div className="flex justify-center">
                        <button
                          ...
                          className="px-8 py-3.5 bg-blue-600/10 border border-blue-500/20 hover:bg-blue-600/20 text-blue-400 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] transition-all flex items-center gap-2"
                        >
                          <Brain className="w-4 h-4" />
                          {showExplanation[currentQuestion] ? "Ocultar Explicação" : "Ver Explicação"}
                        </button>
                      </div>
                      ...
                      <AnimatePresence>
                        {showExplanation[currentQuestion] && (
                          <motion.div
                            ...
                            className="overflow-hidden"
                          >
                            <div className="p-6 bg-blue-500/5 border border-blue-500/10 rounded-[24px] space-y-3 relative group">
                              ...
                              <h4 className="text-[10px] font-black text-blue-500 uppercase tracking-widest flex items-center gap-2">
                                <Brain className="w-3.5 h-3.5" /> Análise do Especialista
                              </h4>
                              <p 
                                className="text-xs sm:text-sm leading-relaxed text-slate-300 font-medium selection:bg-blue-500/30"
                                dangerouslySetInnerHTML={{ __html: formatMarkdown(q.explicacao || q.explicação || q.explicacão) }}
                              />
                            </div>
                          </motion.div>
                        )}
  ...
  const renderFinished = () => {
    ...
    const isGood = aproveitamentoGeral >= 70;
    const circumference = 2 * Math.PI * 54;
    const strokeDashoffset = circumference - (circumference * aproveitamentoGeral / 100);

    return (
      <div className="min-h-screen bg-[#020617] text-slate-200 relative overflow-x-hidden">
        {/* Background glows */}
        <div className="absolute -top-60 -left-60 w-[700px] h-[700px] rounded-full pointer-events-none opacity-60"
          style={{ background: isGood ? 'radial-gradient(circle, rgba(16,185,129,0.07) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(59,130,246,0.07) 0%, transparent 70%)' }} />
        <div className="absolute -bottom-60 -right-60 w-[700px] h-[700px] rounded-full pointer-events-none opacity-60"
          style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.06) 0%, transparent 70%)' }} />

        {/* Top bar */}
        <div className="relative z-10 border-b border-white/[0.04] px-6 sm:px-12 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 bg-blue-600 rounded-lg flex items-center justify-center">
              <GraduationCap className="w-4 h-4 text-white" />
            </div>
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-600">Elite Banker · Resultado</span>
          </div>
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-black uppercase tracking-widest ${
            isGood ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400'
          }`}>
            <div className={`w-1.5 h-1.5 rounded-full ${isGood ? 'bg-emerald-400' : 'bg-blue-400'} animate-pulse`} />
            {isGood ? 'Aprovado' : 'Simulado Concluído'}
          </div>
        </div>

        <div className="relative z-10 max-w-6xl mx-auto px-6 sm:px-12 py-10 sm:py-14">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-10">
            ...
            <div className="lg:col-span-2 flex flex-col gap-6">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-500/60 mb-3">Protocolo Concluído</p>
                <h1 className="text-3xl sm:text-4xl font-light text-white tracking-tighter leading-tight">
                  {isGood ? 'Bom' : 'Continue'}{' '}
                  <span className={`font-black italic ${isGood ? 'text-emerald-400' : 'text-slate-600'}`}>
                    {isGood ? 'Desempenho!' : 'Focado.'}
                  </span>
                </h1>
                <p className="text-slate-500 text-sm mt-2">
                  {isGood ? 'Você está no caminho certo para a aprovação.' : 'Cada erro é uma oportunidade de aprendizado.'}
                </p>
              </div>
              ...
              <div className="relative flex items-center justify-center py-4">
                <div className="relative w-44 h-44">
                  <div className={`absolute inset-0 rounded-full blur-2xl opacity-20 ${isGood ? 'bg-emerald-500' : 'bg-blue-500'}`} />
                  <svg className="w-full h-full -rotate-90 relative z-10" viewBox="0 0 120 120">
                    <circle cx="60" cy="60" r="54" stroke="currentColor" strokeWidth="6" fill="transparent" className="text-white/5" />
                    <motion.circle
                      cx="60" cy="60" r="54"
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                    ...
                    <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1">de {questoesProcessadas.length}</span>
                    <span className={`text-lg font-black mt-1 ${isGood ? 'text-emerald-400' : 'text-blue-400'}`}>
                      {Math.round(aproveitamentoGeral)}%
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 bg-[#0B1224] border border-white/[0.05] rounded-2xl">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Nota Final</p>
                  <p className="text-2xl font-black text-blue-400 tracking-tighter">{pontuacaoTotal.toFixed(1)}</p>
                  <p className="text-[9px] text-slate-600 font-bold">/ {pontuacaoMaxima.toFixed(1)} pts</p>
                </div>
                <div className="p-4 bg-[#0B1224] border border-white/[0.05] rounded-2xl">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Erros</p>
                  <p className="text-2xl font-black text-red-400/80 tracking-tighter">{questoesProcessadas.length - totalAcertos}</p>
                  <p className="text-[9px] text-slate-600 font-bold">questões</p>
                </div>
                <div className="p-4 bg-[#0B1224] border border-white/[0.05] rounded-2xl">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Acertos</p>
                  <p className="text-2xl font-black text-emerald-400/80 tracking-tighter">{totalAcertos}</p>
                  <p className="text-[9px] text-slate-600 font-bold">questões</p>
                </div>
                <div className="p-4 bg-[#0B1224] border border-white/[0.05] rounded-2xl">
                  <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Aproveit.</p>
                  <p className={`text-2xl font-black tracking-tighter ${isGood ? 'text-emerald-400' : 'text-yellow-400'}`}>{Math.round(aproveitamentoGeral)}%</p>
                  <p className="text-[9px] text-slate-600 font-bold">geral</p>
                </div>
              </div>
            </div>

            {/* RIGHT — Disciplines table */}
            <div className="lg:col-span-3">
              <div className="bg-[#0B1224] border border-white/[0.05] rounded-[28px] overflow-hidden h-full flex flex-col">
                <div className="px-6 py-5 border-b border-white/[0.04] flex items-center justify-between shrink-0">
                  <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Desempenho por Disciplina</h3>
                  <span className="text-[9px] font-black text-slate-700 uppercase tracking-widest">{stats.length} matérias</span>
                </div>

                <div className="flex-1 overflow-y-auto divide-y divide-white/[0.03] custom-scrollbar">
                  {stats.map((s: any, i: number) => {
                    const barColor = s.percent >= 70 ? 'bg-emerald-500' : s.percent >= 50 ? 'bg-yellow-500' : 'bg-red-500';
                    const textColor = s.percent >= 70 ? 'text-emerald-400' : s.percent >= 50 ? 'text-yellow-400' : 'text-red-400';
                    return (
                      <motion.div
                        key={i}
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-semibold text-slate-300 group-hover:text-white transition-colors truncate max-w-[60%]">
                            {s.disciplina}
                          </span>
                          <div className="flex items-center gap-3 shrink-0">
                            <span className="text-[10px] font-bold text-slate-500 font-mono">{s.acertos} / {s.total}</span>
                            <span className={`text-sm font-black w-12 text-right ${textColor}`}>{Math.round(s.percent)}%</span>
                          </div>
                        </div>
                ...
                <div className="px-6 py-5 border-t border-white/[0.06] bg-[#020617]/30 flex items-center justify-between shrink-0">
                  <div>
                    <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest">Nota Final · Pesos Oficiais</p>
                    <p className="text-[9px] text-slate-700 mt-0.5">Critérios Cesgranrio · Banco do Brasil</p>
                  </div>
                  <div className="text-right">
                    <span className="text-3xl font-black text-blue-400 tracking-tighter">{pontuacaoTotal.toFixed(1)}</span>
                    <span className="text-slate-700 text-sm font-bold ml-1">/ {pontuacaoMaxima.toFixed(1)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-8">
            <button
              ...
              className={`flex-1 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 border ${
                showReview
                  ...
                  : 'bg-white/[0.03] border-white/[0.07] text-slate-300 hover:bg-white/[0.06] hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className="flex-1 py-4 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-all flex items-center justify-center gap-2"
            >
            ...
            <button
              onClick={() => router.push('/')}
              className="flex-1 py-4 bg-white/[0.03] text-slate-400 border border-white/[0.07] rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/[0.07] hover:text-white transition-all flex items-center justify-center gap-2"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              Início
            </button>
            ...
            {showReview && (
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-sm font-bold text-white">Análise de Questões</h3>
                  <div className="flex p-1 bg-[#0B1224] rounded-xl border border-white/[0.06]">
                    <button
                      onClick={() => setReviewFilter('all')}
                      className={`px-5 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${reviewFilter === 'all' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                    >Todas</button>
                    <button
                      onClick={() => setReviewFilter('wrong')}
                      className={`px-5 py-2 text-[9px] font-black uppercase tracking-widest rounded-lg transition-all ${reviewFilter === 'wrong' ? 'bg-red-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                    >Erros</button>
                  </div>
                </div>
                  ...
                  {questoesProcessadas.map((q: any, idx: number) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className={`rounded-[20px] border overflow-hidden ${isCorrect ? 'border-emerald-500/10' : 'border-red-500/10'}`}
                      >
                        <div className={`flex items-center justify-between px-5 py-3 ${isCorrect ? 'bg-emerald-500/5' : 'bg-red-500/5'}`}>
                          <div className="flex items-center gap-3">
                            <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[9px] font-black ${isCorrect ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                              {idx + 1}
                            </div>
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{q.disciplina}</span>
                          </div>
                          {isCorrect ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <X className="w-4 h-4 text-red-500" />}
                        </div>
                        <div className="px-5 py-4 bg-[#0B1224]">
                          <p className="text-sm text-slate-300 leading-relaxed mb-4">{q.texto}</p>
                          <div className="space-y-2">
                            {Object.entries(q.alternativas).map(([letter, text]: any) => {
                              ...
                              let style = 'bg-white/[0.02] border-white/[0.05] text-slate-500';
                              if (isCorrectAnswer) style = 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300';
                              if (isUserChoice && !isCorrectAnswer) style = 'bg-red-500/10 border-red-500/20 text-red-300';
                              return (
                                <div key={letter} className={`flex items-start gap-3 p-3 rounded-xl border text-[13px] transition-all ${style}`}>
