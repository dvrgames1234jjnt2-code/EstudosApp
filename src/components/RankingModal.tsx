      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-xl bg-[#0B1224]/80 border border-white/5 rounded-[48px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.6)] overflow-hidden flex flex-col max-h-[85vh] backdrop-blur-3xl"
          >
            ...
            <header className="relative p-10 flex items-center justify-between">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 bg-gradient-to-br from-yellow-500/20 to-orange-500/20 rounded-2xl flex items-center justify-center border border-yellow-500/10 shadow-inner">
                  <Trophy className="w-7 h-7 text-yellow-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.4)]" />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-white tracking-tighter uppercase italic">Ranking <span className="text-blue-500">Global</span></h3>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1 line-clamp-1 max-w-[250px]">{simuladoTitle}</p>
                </div>
              </div>
              <button 
                onClick={onClose} 
                className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/10 transition-all active:scale-90"
              >
                <X className="w-5 h-5" />
              </button>
            </header>
            ...
            <div className="flex-1 overflow-y-auto px-10 pb-10 custom-scrollbar relative">
              {loading ? (
                <div className="flex flex-col items-center justify-center py-24 gap-6">
                  ...
                  <p className="text-[10px] font-black text-blue-500/60 uppercase tracking-[0.4em] animate-pulse">Processando Inteligência...</p>
                </div>
              ) : ranking.length > 0 ? (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-1">
                                <span className={`text-[9px] font-black uppercase tracking-widest ${isFirst ? 'text-yellow-500/60' : 'text-slate-600'}`}>
                                  Aproveitamento: {Math.round((row.score/row.total_questions)*100)}%
                                </span>
                                <div className="w-1 h-1 rounded-full bg-slate-800" />
                                <span className="text-[9px] font-black text-slate-700 uppercase tracking-widest">Consolidado</span>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="text-right pr-2">
                          ...
                          <div className="text-[9px] font-black text-slate-700 uppercase tracking-widest mt-0.5">Pontos</div>
                        </div>
                      </motion.div>
                    );
                  })}
                ...
                <div className="text-center py-24 space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[10px] text-slate-800 font-bold uppercase tracking-widest">Aguardando as primeiras execuções de elite.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <footer className="relative p-8 bg-black/20 border-t border-white/[0.05] flex items-center justify-center gap-4">
              ...
              <p className="text-[9px] font-black text-slate-700 uppercase tracking-[0.4em] whitespace-nowrap">Elite Banker Intelligence</p>
