              ...
              {authStep === 'success' ? (
                ...
                <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  <div className="flex p-1 bg-white/[0.02] border border-white/5 rounded-xl mb-6">
                    ...
                    <button onClick={() => setAuthMode('otp')} className={`flex-1 py-2 text-[10px] sm:text-xs font-black uppercase tracking-widest rounded-lg transition-all ${authMode==='otp' ? 'bg-blue-600 text-white' : 'text-slate-500'}`}>Código</button>
                  </div>
